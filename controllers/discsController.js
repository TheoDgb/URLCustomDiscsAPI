...
async function handleCreateCustomDisc(body, res) {
    ...
    try {
        ...
        if (!info.duration || info.duration > MAX_AUDIO_DURATION) {
...
exports.createCustomDiscFromMp3 = async (req, res) => {
    ...
    if (!mp3File) {
        return res.status(400).json({
            success: false,
            error: 'Missing MP3 file.'
        });
    }

    // Inject the file path into the body to pass to handler
    const body = {
        discName,
        audioType,
    ...
    enqueue(token, () => handleCreateCustomDiscFromMp3(body, res)).catch(err => {
        console.error('[QUEUE ERROR]', err);
        res.status(500).json({
            success: false,
            error: 'Internal queue error.'
...
async function handleCreateCustomDiscFromMp3(body, res) {
    ...
    try {
        let info;
        try {
            info = await audioManager.getMp3AudioInfo(mp3FilePath);
        } catch (err) {
            console.error('[AUDIO INFO ERROR]', err);
        ...
        if (!info.filesize || info.filesize > MAX_AUDIO_FILE_SIZE) {
            return res.status(409).json({
                success: false,
                error: !info.filesize
                    ? 'Unable to determine audio file size.'
        ...
        try {
            let oggPath;
            try {
                oggPath = await audioManager.convertMp3Audio(mp3FilePath, discName, audioType, tempAudioDir);
            } catch (err) {
                console.error('[AUDIO ERROR]', err);
            ...
            if (mp3FilePath && fs.existsSync(mp3FilePath)) {
                try {
                    ...
                    console.warn('[CLEANUP ERROR] Failed to delete uploaded MP3 file:', err.message);
                }
            }
        }
